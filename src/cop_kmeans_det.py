...
def cop_kmeans_det(dataset,current_f, k, ml=[], cl=[],
               initialization='kmpp',
    ...
    for _ in range(max_iter):
        ...
        '''
        # if cluster initialize when already initialized tracker violate constraints
        if k - len(set(clusters_))==1:
            centers_ = centers_[0:-1]
            k = len(set(clusters_))
        '''
        # TODO: cluster center may remain unassociated? len(clusters_)<k
        #assert (len(centers) - k == 1) or (len(centers) - k == 0), 'total number of center with died tracker - k >1'
        if len(set(clusters_))<k: # cluster center fails to associate
            k = len(set(clusters_))

        clusters_, centers_,k,knew = compute_centers(clusters_, dataset, k, ml_info)
...
def compute_centers(clusters, dataset, k, ml_info):
    ...
    return clusters, centers,k,k_new # return only k centers
