...
def temporal_diff_test(cluster_Q,cluster_i_embed,cluster_center,ID_ind):
    uni, cnts = np.unique(cluster_Q[:, 0], return_counts=True)
    temporal_incons_fr = uni[np.where(cnts > 1)]
    if(len(temporal_incons_fr))>0:
        for i in range(len(temporal_incons_fr)):
            noisy_samples = cluster_Q[np.where(cluster_Q[:,0]==temporal_incons_fr[i])]
            noisy_samples_embed = cluster_i_embed[np.where(cluster_Q[:, 0] == temporal_incons_fr[i])]
            dist = pairwise_distances(noisy_samples_embed, cluster_center.reshape(1,32))
            dist_avg = np.mean(dist,axis=1)
            noisy_samples[:,8][np.where(dist_avg>dist_avg.min())] = 0 # noisy sample keep unassociated
            cluster_Q[np.where(cluster_Q[:,0]==temporal_incons_fr[i])]=noisy_samples
    return cluster_Q
...
def cluster_association(fr,X_frames,latent_feature, det_frame,det_at_t_pmask, labels,
                        cluster_center, ID_ind, time_lag,score_th,min_cluster_size):
    final_det = []
    final_mask = []
    ...
    for j in labels_unique: #cluster wise loop
        ...
        x_frames = cluster_Q[:,0].astype(np.float32)
        ...
        if (sum(cluster_Q[:, 8]) == 0 and cluster_size <= time_lag and
                len(np.unique(cluster_Q[:,0])) >= min_cluster_size and len(np.where(avg_iou>=0.2)[0])>=min_cluster_size):
           ...
           cluster_Q[:, 8][np.where(avg_iou>=0.2)] = ID_ind  # initialize all detections in a cluster with cluster ID
        ...
        # Case2: All samples are unassociated [cluster size > temporal window]
           ...
           center_cwc,label_cwc, inertia_cwc,label_cwc_noise = k_means(cluster_i_embed,x_frames, n_clusters=count_cwc.max())
           #labels_unique_cwc, counts_cwc = np.unique(label_cwc, return_counts=True)
           #label_cwc = label_cwc_noise
           for k in np.unique(label_cwc):
               ...
               cluster_cwc_embed= cluster_i_embed[np.where(label_cwc==k)]
               ...
               iou_matrix_non_diag = pairwise_iou_non_diag(boxs)
               avg_iou = np.mean(iou_matrix_non_diag, axis=1)
               if(len(np.unique(cluster_cwc[:,0]))>=min_cluster_size and len(np.where(avg_iou>=0.2)[0])>=min_cluster_size):#sum(cluster_cwc[:,6])/time_lag >=score_th
                  print('Cluster Label Before Association',cluster_cwc[:,8])
                  cluster_cwc[:,8][np.where(avg_iou>=0.2)] = ID_ind  # initialize all detections in a cluster with cluster ID
                  #cluster_cwc = temporal_diff_test(cluster_cwc, cluster_cwc_embed, center_cwc[k], ID_ind)
                  ...
                  final_mask.append(refined_mask.reshape(28, 28))
                  refined_det[6] = np.sum(cluster_cwc[:, 6]) / time_lag
                  final_det.append(refined_det)
                  print('new trajectory', ID_ind)
           ...
           #
           # TODO: Delete bad (unassociated) samples from the cluster_Q and also the corresponding labels from the label sets
           #
           '''
           if len(np.where(avg_iou<0.3)[0])>0:
               cluster_Q = np.delete(cluster_Q,list(cluster_label).index(0),axis=0)
               labels = np.delete(labels,[np.where(labels==j)][list(cluster_label).index(0)],axis=0)
               print('Unassociated samples deleted from trajectory-', cluster_Q[:,0])
           '''
        #
        ...
        # Use cluster_new = [0,1] (in all zero case) identifier to consider new cluster with no zero id
            ...
            if(len(cluster_Q)>time_lag):
                ...
                for k in np.unique(label_cwc):
                    ...
                    if (len(np.unique(cluster_cwc[:,0])) >= min_cluster_size and len(np.where(avg_iou>=0.2)[0])>=min_cluster_size):  # sum(cluster_cwc[:,6])/time_lag >=score_th
                        ...
                        if ( sum(cluster_cwc_label)==0 and sum(cluster_cwc[:, 6])/time_lag >=score_th):  # New object in FOV
                            cluster_cwc[:, 8][np.where(avg_iou >= 0.2)] = ID_ind
                            #cluster_cwc = temporal_diff_test(cluster_cwc, cluster_cwc_embed, center_cwc[k], ID_ind)
                            print('new trajectory', ID_ind)
                            print('new trajectory labels after association', cluster_cwc[:, 8])
                            ID_ind += 1
                            ...
                            cluster_cwc[:, 8][np.where(avg_iou >= 0.2)] = uni[np.where(cnt == cnt.max())].max()
                            #cluster_cwc = temporal_diff_test(cluster_cwc, cluster_cwc_embed, center_cwc[k], uni[np.where(cnt == cnt.max())].max())
                ...
                if(len(cluster_label[np.where(cluster_label==0)])>=min_cluster_size and len(cluster_Q[:, 8][np.where(avg_iou>=0.2)])>0
                        and uni[np.where(cnt == cnt.max())].max()==0 and sum(cluster_Q[:, 6])/time_lag >=score_th
                        and len(np.unique(cluster_Q[:,0])) >= min_cluster_size):
                    # Initiate new object trajectroy when all IDs are zero with cluster score>=0.5
                    cluster_label[np.where(avg_iou >= 0.2)] = ID_ind  # cluster_label[np.where(avg_iou == avg_iou.max())][0] uni[np.argmax(cnt)]
                    ...
                    ID_ind += 1
                elif(len(np.unique(cluster_Q[:,0])) >= min_cluster_size):
                    non_zero_id = cluster_label[cluster_label.nonzero()]
                    uni, cnt = np.unique(non_zero_id, return_counts=True)
                    cluster_label[np.where(avg_iou >= 0.2)] = uni[np.where(cnt == cnt.max())].max()
                ...
                print('After Association:', cluster_Q[:,8])
                det_frame[np.where(labels == j), :] = cluster_Q
    ...
    return np.array(final_det), final_mask, det_frame, ID_ind
...
def cluster_association_constraints_old(fr, X_frames, latent_feature, det_frame, det_at_t_pmask,
                                    labels, ID_ind, time_lag, score_th, min_cluster_size,
                                    k_value, names, color, mask_y, mask_x, im_shape, trackers):
    #---------------------------------------------------------------------------
    # Used in: TCT_KITTI
    ...
    labels_unique, counts = np.unique(labels, return_counts=True)
    print('Number of estimated cluster',len(labels_unique))
    for j in labels_unique[labels_unique>=0]: #cluster wise loop
        ...
        if j>0 or j==0:
            ...
            if(fr==48):
                img = plot_cluster(cluster_Q, names, color[j])
                im = Image.fromarray(img)
                im.save('/media/RemoteServer/CLASP2/2019_04_16/exp2/Results/cam09exp2.mp4/tracking_results_aug/cluster_on_image' +'/cluster'+str(j)+'_'+ names[-10:])
            ...
            uni, cnt = np.unique(cluster_label, return_counts=True)
            # [0,0,0,0,0] :
            if (sum(cluster_Q[:, 8]) == 0 and cluster_size <= time_lag and cluster_size>=min_cluster_size and tracker_update == 0):
               # All cluster ssamples are unassociated (zero id)
               # ID should not be updated for outlier
               #
               # Cluster sample analysis: good - initialized new id or bad - TODO: robust method to separate good and bad sample
               #
               print('average_IoU of cluster samples:',avg_iou)
               #cluster_Q[:, 8][np.where(avg_iou>=0.1)] = ID_ind
               cluster_Q[:, 8] = ID_ind  # initialize all detections in a cluster with cluster ID
               print('new trajectory', ID_ind)
               print('new trajectory labels after association', cluster_Q[:, 8])
               #print('Frame Pattern',cluster_Q[:, 0])
               voted_id = int(ID_ind)
               refined_det = cluster_Q[np.where(cluster_Q[:,0]==cluster_Q[:,0].max())][0]
               refined_mask = cluster_i_mask[np.where(cluster_Q[:, 0] == cluster_Q[:, 0].max())][0]
               ...
               refined_det[6] = np.sum(cluster_Q[:, 6]) / time_lag
               #TODO: previous frame target considered at current frame?
               refined_det[0] = fr
               final_det.append(refined_det)
               det_frame[np.where(labels == j), :] = cluster_Q
            ...
            #len(cluster_label[np.where(avg_iou>=0.2)])>=min_cluster_size and
                ...
                if(len(cluster_label[np.where(cluster_label==0)])>=min_cluster_size
                    ...
                    cluster_label[np.where(cluster_label==0)] = ID_ind
                    print('new trajectory labels before association', cluster_Q[:,8])
                    #cluster_label[::]=ID_ind
                    print('new trajectory', ID_ind)
                    print('new trajectory labels after association', cluster_label)
                ...
                # [6 6 6 0 0]?? or #[6 6 6 6 0]-yes
                    ...
                    print('labels before association', cluster_Q[:,8])
                    print('labels after association', cluster_label)
                    refined_det[0] = fr
                    refined_det[6] = np.sum(cluster_Q[:, 6]) / time_lag
                    # save all initialized tracklets box+rle
            ...
            elif(cluster_size>=min_cluster_size and cluster_size<=time_lag and  tracker_update==0):
    ...
    return np.array(final_det), final_mask, det_frame, ID_ind, trackers
...
def cluster_association_constraints_mnist(fr,latent_feature, det_frame, det_at_t_pmask,
